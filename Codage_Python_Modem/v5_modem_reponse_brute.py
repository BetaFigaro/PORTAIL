...
def hang_up(modem):
    ...
    modem.write(b'ATH0\r')  # Envoi de la commande AT pour raccrocher (ou ATH0)
    time.sleep(2)  # Attendre 2 secondes pour s'assurer que le modem a bien raccroché
    ...
    while True:
        try:
            ...
            if response.startswith('+CLIP'):
                ...
                # Vérifier si le numéro contient +33 et le retirer si c'est le cas
                ...
                time.sleep(5)  # Attendre 5 secondes pour que l'appel soit pris en compte

                # Raccrocher la ligne après l'envoi
                hang_up(modem)

        except Exception as e:
            print(f"Erreur lors du traitement de la réponse du modem : {e}")
